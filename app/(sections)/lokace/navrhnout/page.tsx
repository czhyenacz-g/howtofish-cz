...
import { submitLocationCorrectionAction } from "./actions";
import LocationSuggestionForm from "./LocationSuggestionForm";

export const metadata: Metadata = {
  title: "Navrhnout novou lokaci",
  robots: { index: false, follow: false },
};

export default async function NavrhnoutLokacePage({
  searchParams,
}: {
    ...
    <div className="bg-gradient-to-b from-[#0a2438] via-[#0e4f66] to-[#146b78] px-4 py-12 text-white">
      <div className="mx-auto max-w-xl">
        ...
        <h1 className="mt-4 font-serif text-3xl">
          {correctionTarget ? "Navrhnout opravu" : "Navrhnout novou lokaci"}
        </h1>
        <p className="mt-2 text-cyan-100/80">
          {correctionTarget
            ? "Něco u této lokace nesedí? Napiš nám, co se má opravit — administrátor to posoudí."
            : "Narazil jsi na lokaci, kterou tu ještě nemáme? Pošli nám návrh — po ověření ji přidáme."}
        </p>

        {!user ? (
          <div className="mt-8 rounded-lg border border-amber-400/40 bg-amber-400/10 p-5 text-center">
            <p className="text-amber-200">
              {correctionTarget ? "Pro návrh opravy se přihlas přes Steam." : "Pro návrh nové lokace se přihlas přes Steam."}
            </p>
            <Link
              href={`/api/auth/steam/login?returnTo=${encodeURIComponent(
                correctionTarget ? `/lokace/navrhnout?correction=${encodeURIComponent(correctionTarget)}` : "/lokace/navrhnout"
              )}`}
          ...
          <div className="mt-8">
            {correctionTarget ? (
              <CorrectionForm target={correctionTarget} action={submitLocationCorrectionAction} />
            ) : (
