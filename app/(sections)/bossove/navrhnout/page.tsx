...
import { submitBossCorrectionAction } from "./actions";
import BossSuggestionForm from "./BossSuggestionForm";

export const metadata: Metadata = {
  title: "Navrhnout nového bosse",
  robots: { index: false, follow: false },
};

export default async function NavrhnoutBossePage({
  searchParams,
}: {
    ...
    <div className="bg-gradient-to-b from-[#0a2438] via-[#0e4f66] to-[#146b78] px-4 py-12 text-white">
      <div className="mx-auto max-w-xl">
        ...
        <h1 className="mt-4 font-serif text-3xl">
          {correctionTarget ? "Navrhnout opravu" : "Navrhnout nového bosse"}
        </h1>
        <p className="mt-2 text-cyan-100/80">
          {correctionTarget
            ? "Něco u tohoto bosse nesedí? Napiš nám, co se má opravit — administrátor to posoudí."
            : "Narazil jsi na bosse, kterého tu ještě nemáme? Pošli nám návrh — po ověření ho přidáme."}
        </p>

        {!user ? (
          <div className="mt-8 rounded-lg border border-amber-400/40 bg-amber-400/10 p-5 text-center">
            <p className="text-amber-200">
              {correctionTarget ? "Pro návrh opravy se přihlas přes Steam." : "Pro návrh nového bosse se přihlas přes Steam."}
            </p>
            <Link
              href={`/api/auth/steam/login?returnTo=${encodeURIComponent(
                correctionTarget ? `/bossove/navrhnout?correction=${encodeURIComponent(correctionTarget)}` : "/bossove/navrhnout"
              )}`}
          ...
          <div className="mt-8">
            {correctionTarget ? (
              <CorrectionForm target={correctionTarget} action={submitBossCorrectionAction} />
            ) : (
              <BossSuggestionForm />
            )}
          </div>
